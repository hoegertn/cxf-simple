...
public class DemoTest1 extends BasicTest {
	
	@Test
	public void testNotExisting() {
		HttpResponse get = this.request("/demo/{name}").pathParam("name", "nx").get();
	...
	public void testCreate() {
		...
		HttpResponse post = this.json(this.request("/demo"), t1).post();
		System.out.println(post);
		Assert.assertEquals(201, WS.getStatus(post));
		
		HttpResponse get = this.request("/demo/{name}").pathParam("name", "foo").get();
		System.out.println(get);
	...
	public void testCreateDuplicate() {
		...
		HttpResponse post2 = this.json(this.request("/demo"), t1).post();
